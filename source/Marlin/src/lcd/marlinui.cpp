...
void MarlinUI::tempload(){
      ...
      queue.enqueue_now_P(PSTR("M109 S230"));
}
...
void MarlinUI::CompletedMaticHate(void){
    if(load_flag){
        LCD_MESSAGEPGM(MSG_FILAMENT_CHANGE_LOAD);
        gcode.process_subcommands_now(PSTR("G92 Z0 E0"));
        gcode.process_subcommands_now(PSTR("G1 F800 Z20"));
        gcode.process_subcommands_now(PSTR("G1 F500 E350"));
        gcode.process_subcommands_now(PSTR("G1 F100 E500"));
        gcode.process_subcommands_now(PSTR("M400"));
        gcode.process_subcommands_now(PSTR("M104 S0"));
    }
    else if(unload_flag){
        LCD_MESSAGEPGM(MSG_FILAMENT_CHANGE_UNLOAD);
        gcode.process_subcommands_now(PSTR("G92 Z0 E0"));
        gcode.process_subcommands_now(PSTR("G1 F800 Z10"));
        gcode.process_subcommands_now(PSTR("G1 F500 E50"));
        gcode.process_subcommands_now(PSTR("G1 F800 E-600")); 
        gcode.process_subcommands_now(PSTR("M400"));
        gcode.process_subcommands_now(PSTR("M104 S0"));    
    }
}

void MarlinUI::abort_load(){
      ...
      if (old_status < 2) {
        TERN_(EXTENSIBLE_UI, ExtUI::onMediaRemoved()); // ExtUI response
          ...
          LCD_MESSAGEPGM(MSG_MEDIA_REMOVED);
            ...
            if(printingIsActive()&&!load_flag && !unload_flag){
