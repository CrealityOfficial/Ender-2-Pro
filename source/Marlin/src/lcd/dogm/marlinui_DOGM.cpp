...
#include LANGUAGE_DATA_INCL(LCD_LANGUAGE)    //中文
#include LANGUAGE_DATA_INCL(LCD_LANGUAGE_2) // 英文
#include LANGUAGE_DATA_INCL(LCD_LANGUAGE_8)  //俄语
...
#endif
  // uxg_SetUtf8Fonts(g_fontinfo, COUNT(g_fontinfo));
  ui.set_status(GET_TEXT(WELCOME_MSG));
  ui.refresh();
}
...
void MarlinUI::clear_lcd() { } // Automatically cleared by Picture Loop
  ...
  inline void draw_boxed_string(const u8g_uint_t x, const u8g_uint_t y, PGM_P const pstr, const bool inv) {
    u8g_uint_t len = utf8_strlen_P(pstr);
    if(ui.language == UI_LANGUAGE_CN){
        len+=2;
    }
    const u8g_uint_t by = (y + 1) * (MENU_FONT_HEIGHT);
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    u8g_uint_t len = utf8_strlen_P(yes)+1;
    if(ui.language == UI_LANGUAGE_CN){
        len+=2;
    }
    ...
    draw_boxed_string(LCD_WIDTH - len, LCD_HEIGHT - 1, yes, yesno);
  }
