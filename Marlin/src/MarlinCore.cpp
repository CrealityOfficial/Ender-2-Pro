...
void startOrResumeJob() {
  ...
  inline void pauseSDPrinting()
  {
    if(printingIsPaused() && (!planner.has_blocks_queued()))
    {
      pause_action_flag = false;
      queue.inject_P("G1 F1200 X0 Y0");
    }
  }
...
void loop() {
  do {
    idle();
      ...
      if (pause_action_flag) pauseSDPrinting();
    ...
    if(!SD_Card_status)
    {
      ...
      if(card.isMounted())
      {
        ...
        SERIAL_ERROR_MSG("SD Read Error Fixed...");
        queue.inject_P(PSTR("M24"));
      }
      else
      {
        SD_Card_status = false;
        SERIAL_ERROR_MSG("SD Mount Error...");
      }
      SERIAL_ERROR_MSG("SD Mount Retry...");
      delay(200);
    }
