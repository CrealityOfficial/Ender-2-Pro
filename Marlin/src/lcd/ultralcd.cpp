...
void MarlinUI::update() {
  ...
  void MarlinUI::abort_print()
  {
      ...
      //=====================================================
      // queue.clear();        //此两句打开后会出现第一次正常停止打印后,第二次打印屏卡顿问题     
      // quickstop_stepper();   //Modified by kim 20211019
      //=====================================================
  ...
  {
    if(card.isPrinting() && (thermalManager.temp_hotend[0].celsius > (thermalManager.temp_hotend[0].target - 2)))
    {
      ...
      set_status_P(print_paused);
        ...
        while(planner.movesplanned() && (queue.length >= (BUFSIZE - 3)) && (destination != current_position))
        {
          idle();
        }
  ...
  {
    if(card.isPaused() && (pause_action_flag == false))
    {
      reset_status();
      pause_action_flag = false;
      char cmd[20];
