...
void MarlinUI::clear_lcd() { } // Automatically cleared by Picture Loop
  ...
  inline void draw_boxed_string(const u8g_uint_t x, const u8g_uint_t y, PGM_P const pstr, const bool inv) {
    ...
    const pixel_len_t bw = len * (6), bx = x * (6);
    if (inv)
    {
      u8g.setColorIndex(1);
      ...
      {
        u8g.drawBox(bx * 2 - 1, by - (MENU_FONT_ASCENT) + 1, bw * 2 + 2, MENU_FONT_HEIGHT - 1);
      }
      u8g.setColorIndex(0);
    }
    if(language_change_font == 0)
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ...
    if(language_change_font == 0)
    {
      draw_boxed_string(21 - (utf8_strlen_P(yes) + 1), LCD_HEIGHT - 1, yes, yesno);
    }
    else
    {
      draw_boxed_string(11 - (utf8_strlen_P(yes) + 1), LCD_HEIGHT - 1, yes, yesno);
    }
  }

    void MenuItem_confirm::draw_select_chinese_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ui.draw_select_chinese_screen_prompt(pref, string, suff);
    draw_boxed_string(1, LCD_HEIGHT - 1, no, !yesno);
    if(language_change_font == 0)
    {
      draw_boxed_string(21 - (utf8_strlen_P(yes) + 1), LCD_HEIGHT - 1, yes, yesno);
